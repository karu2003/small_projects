...
#include "pico/time.h"
#include <stdint.h>    // Заменить <cstdint>
#include <stdlib.h>    // Заменить <cstdlib>
#include <stdio.h>
...
static const float    MIN_PULSE_PERIOD_US = MIN_PULSE_PERIOD/2;  // Убрать constexpr
static const float    PIO_FREQ            = SYS_FREQ * 1000.0f;  // Убрать constexpr
static const uint16_t MIN_INTERVAL_CYCLES =
    MIN_PULSE_PERIOD_US * (SYS_FREQ / 1000);                     // Убрать constexpr
static const float    AUDIO_SAMPLE_RATE = 48000.0f;             // Убрать constexpr
static const uint32_t AUDIO_FRAME_TICKS =
    SYS_FREQ * 10.0 / AUDIO_SAMPLE_RATE;                        // Убрать constexpr

// Аудио буферы и параметры
